...
export async function sendTaskEmailNow(id){
  const r = await fetch('/api/tasks/'+id+'/sendNow', { method:'POST' })
  if (!r.ok) throw new Error((await r.json()).error||'Falha ao enviar e-mail')
  return r.json()
}

export async function sendTaskEmailTo(id, email){
  const r = await fetch('/api/tasks/'+id+'/sendTo', {
    method:'POST',
