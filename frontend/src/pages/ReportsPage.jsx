...
export default function ReportsPage(){
  ...
  function downloadXML(){
    // Create XML for transactions in the current table (filtered)
    const xmlHeader = '<?xml version="1.0" encoding="UTF-8"?>\n'
    const items = rows.map(r => (
      `  <movimento id="${r.id}">\n`+
      `    <descricao>${escapeXml(r.description)}</descricao>\n`+
      `    <categoria>${escapeXml(r.category)}</categoria>\n`+
      `    <tipo>${r.type}</tipo>\n`+
      `    <valor>${Number(r.amount).toFixed(2)}</valor>\n`+
      `    <data>${r.date}</data>\n`+
      `  </movimento>`)).join('\n')
    const xml = xmlHeader + `<relatorioFinanceiro inicio="${range.start}" fim="${range.end}">\n` + items + '\n</relatorioFinanceiro>'
    const blob = new Blob([xml], { type:'application/xml' })
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `relatorio-financeiro_${range.start}_a_${range.end}.xml`
    a.click()
    ...
    <div>
      <form className="card" onSubmit={onFilter}>
        ...
        <div className="row">
          ...
          <button type="button" onClick={downloadXML}>Baixar XML</button>
          <button type="button" onClick={downloadTasksPDF}>PDF de Tarefas</button>
